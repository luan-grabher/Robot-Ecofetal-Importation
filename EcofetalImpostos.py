...
# robot with properties 'parameters' to test
robot = lambda: None
robot.parameters = {'mes': '1', 'ano': '2022'}
...
try:
    ...
    path = '\\\\heimerdinger\\docs\\contábil\\clientes\\' + enterprisePath + \
        '\\Escrituração Mensal\\' + \
        str(ano) + '\\' + yearPath + '\\' + str(mes) + '\\' + monthPath
    path = '.'    

    # get first file with path + '\\CONTAS*RECEBIDAS*RETENC*.xlsx'
    file = glob.glob(path + '\\CONTAS*RECEBIDAS*RETENC*.xlsx')[0]
